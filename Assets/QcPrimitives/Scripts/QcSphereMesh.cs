...
namespace QuickPrimitives
{
    ...
    public class QcSphereMesh : QcPrimitivesBase
    {
        ...
        private void GenerateMeshUvFullSphere()
        {
            ...
            AddVertex(Vector3.up * -properties.radius);     // bottom vertex (south pole)
            ...
            if (properties.genTextureCoords)
            {
                ...
                for (int lat = 0; lat < segments; lat++)
                {
                    for (int lon = 0; lon <= segments; lon++)
                    {
                        AddUV(new Vector2((float)lon / segments, 1f - (float)(lat + 1) / (segments + 1)));
                    }
                }
                AddUV(Vector2.zero);
            }
            ...
            for (int lon = 0; lon < segments; lon++)
            {
                faces.Add(new TriangleIndices(lon + 2, lon + 1, 0));
            }
            ...
            if (!sideCap)
            {
                ...
                for (int lon = 0; lon < segments; lon++)
                {
                    faces.Add(new TriangleIndices(vertices.Count - 1,
                                                  vertices.Count - (lon + 2) - 1,
                                                  vertices.Count - (lon + 1) - 1));
                }
            }
            else
            {
                //Bottom Cap
                int baseIndex = segments * (segments + 1) + 2;
                for (int lon = 0; lon < segments; lon++)
                {
                    faces.Add(new TriangleIndices(baseIndex - 1,
                                                  baseIndex - (lon + 2) - 1,
                                                  baseIndex - (lon + 1) - 1));
                }
        ...
        private void GenerateMeshUvHemiSphere()
        {
            ...
            for (int n = segments + 2; n < vertices.Count; n++)
            {
                AddNormal(vertices[n].normalized);
            }
            ...
            for (int lat = 0; lat < vSegments - 1; lat++)
            {
                for (int lon = 0; lon < segments; lon++)
                {
                    int current = lon + (lat + 1) * (segments + 1) + 1;
                    int next = current + (segments + 1);
            ...
            {
                //Bottom Cap
                int baseIndex = (vSegments + 1) * (segments + 1) + 2;
                for (int lon = 0; lon < segments; lon++)
                {
                    faces.Add(new TriangleIndices(baseIndex - 1,
                                                  baseIndex - (lon + 2) - 1,
